...
module sort_sim_tb;
    ...
    // ============================================================
    // Helper: push a string into UART input buffer
    // In Vivado XSim, the first char of a string literal assigned to
    // a packed reg goes to the MOST significant byte. So we must read
    // from the highest byte index (127 for 1024-bit) downwards.
    // ============================================================
    task push_string;
        input [1023:0] s;
        integer j;
        begin
            for (j = 127; j >= 0; j = j - 1) begin
                if (s[j*8 +: 8] == 8'h00) break;
                uart_inbuf[uart_wr_count] = s[j*8 +: 8];
                uart_wr_count = uart_wr_count + 1;
            end
        end
    endtask
    ...
    initial begin
        ...
        // Pre-load UART input for sort test:
        // Phase 0: 8 integers (space-separated)
        // Phase 1: array size (after the first 8 numbers)
        push_string("12 34 56 78 90 12 34 56");
        push_string("\n");
        push_string("10");    // Phase 1 array size
        push_string("\n");
        push_string("11 45 14 19 81 67 19 91 78 13");
        push_string("\n");
