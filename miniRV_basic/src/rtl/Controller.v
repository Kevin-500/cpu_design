...
module Controller (
    ...
    // rf_we
    wire RF_OP_WE = ADDI | ORI | SLLI | SRLI | SRAI | XORI |
                    ADD | SUB | XOR | SLL | SRL | SRA |
                    LB | LBU | LH | LHU | LW | LUI | AUIPC | JAL | JALR |
                    AND | OR | SLT | SLTU | MUL | MULH | MULHU | DIV | DIVU |
                    REM | REMU | ANDI | SLTI | SLTIU;
    
    // rf_wsel
    wire WB_OP_ALU = ADDI | ORI | SLLI | SRLI | SRAI | XORI |
                     ADD | SUB | XOR | SLL | SRL | SRA | AUIPC |
                     AND | OR | SLT | SLTU | MUL | MULH | MULHU | DIV | DIVU |
                     REM | REMU | ANDI | SLTI | SLTIU;
    wire WB_OP_RAM = LB | LBU | LH | LHU | LW;
    wire WB_OP_PC4 = JAL | JALR;
    wire WB_OP_EXT = LUI;
    
    // sext_op
    wire EXT_OP_I = ADDI | ORI | SLLI | SRLI | SRAI | XORI |
                    LB | LBU | LH | LHU | LW | JALR | ANDI | SLTI | SLTIU;
    ...
    // alu_op
    wire ALU_OP_ADD   = ADDI | ADD | LB | LBU | LH | LHU | LW |
                        SB | SH | SW | AUIPC | JALR;
    wire ALU_OP_SUB   = SUB;
    ...
    // alub_sel
    wire ALU_B_SEL_RS2 = ADD | SUB | XOR | SLL | SRL | SRA | BEQ | BNE |
                         AND | OR | SLT | SLTU | MUL | MULH | MULHU | DIV | DIVU |
                         REM | REMU | BLT | BGE | BLTU | BGEU;
    wire ALU_B_SEL_EXT = !ALU_B_SEL_RS2;
