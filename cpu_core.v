...
module cpu_core(
    ...
    /* 保留 pc 这个公开名字，供指导书自带的 liveness/debug testbench 观察。 */
    ...
    MREQ U_MEM_REQ (
        .ram_addr(exmem_alu_result), 
        .ram_rop(exmem_ram_rop),
        .da_ren(mem_req_ren), 
        .da_addr(mem_req_addr),
        .ram_wop(exmem_ram_wop), 
        .ram_wdata(exmem_store_value),
        .da_wen(mem_req_wen), 
        .da_wdata(mem_req_wdata)
    );
    ...
    MEXT U_MEM_EXT (
        .op(exmem_ram_rop), 
        .din(daccess_rdata),
        .byte_offs(exmem_alu_result[1:0]), 
        .ext(load_value)
    );
    ...
    /* load 返回值可能比乘除法指令进入 ID/EX 晚一个周期才到达前递网络。
       因此乘除法启动前先等操作数稳定，再给多周期 ALU 一个单周期 start 脉冲。 */
    ...
    always @(posedge cpu_clk or posedge cpu_rst) begin
        if (cpu_rst) begin
            ...
            /* WB 是单周期提交级：默认本拍无写回，只有 MEM 完成时再拉高。 */
            memwb_valid <= 1'b0;

            /* 接收一条取指返回。
               如果这条返回属于已被 EX 阶段跳转冲刷掉的路径，只消费它，不写入 IF/ID。 */
            ...
            /* 乘除法先让 ID/EX 保持一个操作数稳定周期，然后只启动一次多周期 ALU。 */
            if (idex_valid && idex_is_md && !md_started) begin
                /* 多周期运算期间 ID/EX 会保持。
                   若此时有 load 结果只前递一个周期，需要把它吸收到 ID/EX 源操作数寄存器里，
                   避免“前一条 load 的值先到、后一条 load 的值后到”时丢掉早到的值。 */
            ...
            /* 普通非乘除法指令也可能因为更老的 load/store 卡在 MEM 而停在 ID/EX。
               暂停期间若出现一拍前递值，同样要吸收到 ID/EX 中；
               否则类似 “lw; lw; branch/use-both” 的序列会在第二个 load 返回前丢掉第一个 load 的值。 */
            if (idex_valid && !idex_is_md && !ex_fire) begin
                if (exmem_forward_valid && (exmem_rd == idex_rs1))
                    idex_rs1_value <= exmem_result;
                else if (memwb_forward_valid && (memwb_rd == idex_rs1))
                    idex_rs1_value <= memwb_value;
            ...
            /* EX 结果已完成且 MEM 可接收时，EX 才能推进到 EX/MEM。 */
            if (ex_fire) begin
                exmem_valid <= 1'b1;
                exmem_pc <= idex_pc;
